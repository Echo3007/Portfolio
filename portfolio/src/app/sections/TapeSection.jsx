...
export const TapeSection = () => {
    ...
    useEffect(() => {
        // Slight delay + requestAnimationFrame for smoother initial render
    ...
    return (
        <div className="py-16 overflow-hidden relative">
            <div className="bg-gradient-to-r from-emerald-300 to-sky-400 -rotate-3 -mx-1 relative">
                ...
                <div className="flex">
                    <div
                        className="flex flex-none gap-4 py-3 lg:gap-6 min-w-max animate-move-left will-change-transform"
                        style={{ animationDuration: "120s" }}
                    >
                        {repeatedWords.map((word, index) => (
                            <div key={index} className="inline-flex gap-4 items-center">
                                <span className="text-gray-800 uppercase font-extrabold text-sm whitespace-nowrap">
                                    {word}
                                </span>
                                <svg
                                    className="w-6 h-6 text-gray-800 dark:text-white -rotate-12"
